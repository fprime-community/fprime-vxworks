...
void VxWatchDogTimer::startTimer(FwSizeType microseconds) {
    ...
    this->m_tickDelay = static_cast<_Vx_ticks_t>(((microseconds * clkRate) + (USECS_PER_SECS - 1)) / USECS_PER_SECS);
...
void VxWatchDogTimer::startTimer(Fw::TimeInterval interval) {
    ...
    FW_ASSERT(
        interval.getSeconds() <= (std::numeric_limits<FwSizeType>::max() - interval.getUSeconds()) / USECS_PER_SECS,
        static_cast<FwAssertArgType>(interval.getSeconds()), static_cast<FwAssertArgType>(interval.getUSeconds()));
